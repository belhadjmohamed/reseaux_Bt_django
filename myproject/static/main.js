...
const linesLayerGroup = L.layerGroup();
const postesLayerGroup = L.layerGroup();
const departLayerGroup = L.layerGroup();
const derivationLayerGroup = L.layerGroup();
const ancragesLayerGroup = L.layerGroup();
const comptageLayerGroup = L.layerGroup();
...
function init(){
    ...
    //overlay object 
    const overlayers = {
        'supports': markersLayerGroup,
        'branchements': linesLayerGroup,
        'Postes': postesLayerGroup,
        'Departs': departLayerGroup,
        'Derivations': derivationLayerGroup,
        'Ancrages': ancragesLayerGroup,
        'Comptages': comptageLayerGroup,
    }
    ...
    for (var x =0 ; x < postes.length; x++){
        ...
        postesLayerGroup.addLayer(postes_leaflet)
    }
    ...
    for (var dep = 0 ; dep < depart.length; dep++){
        ...
        departLayerGroup.addLayer(ligne_depart)
    ...
    for (var der = 0 ; der < derivation.length; der++){
        ...
        derivationLayerGroup.addLayer(ligne_derivation)
    }
    ...
    for (var anc = 0; anc < ancrages.length; anc++) {
        ...
        ancragesLayerGroup.addLayer(ancrage)
    }
    ...
    for (var com = 0; com < comptages.length; com++) {
        ...
        comptageLayerGroup.addLayer(comptage)
    }

    var button = document.getElementById("id_button");
    var input = document.getElementById("input_search");
    button.addEventListener("click", function() {
        var searchedPostName = input.value.trim();

        
        var searchedLayer = postesLayerGroup.getLayers().find(function(layer) {
            return layer._popup._content.includes(searchedPostName);
        });

        if (searchedLayer) {
            // Fit the map to the bounds of the searched polygon
            mymap.fitBounds(searchedLayer.getBounds());
        } else {
